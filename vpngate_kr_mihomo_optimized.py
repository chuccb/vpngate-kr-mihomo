...
def _validate_v72_config(config: dict[str, Any]) -> None:
    groups = config.get("proxy-groups")
    if not isinstance(groups, list) or len(groups) != 1 or not isinstance(groups[0], dict):
        raise RuntimeError("KR-LOWEST proxy group missing")
    group = groups[0]
